...
public class CharacterController : MonoBehaviour
{
	...
	public Direction Move(float move, float verticalMove, bool crouch, bool jump)
	{
		...
		if (m_Grounded && jump && !hasJumped)
		{
			...
			m_Grounded = false;
			m_Rigidbody2D.AddForce(new Vector2(0f, calculateForce(Physics2D.gravity.magnitude, fixedJumpHeight)), ForceMode2D.Force);
			hasJumped = true;
			freezeUp = false;
		}
		if (!m_Grounded && !jump && !freezeUp && Vector2.Dot(m_Rigidbody2D.velocity, Vector2.up) > 0)
		{
			Vector2 targetVelocity = new Vector2(move * 10f, 0.0f);
			freezeUp = true;
